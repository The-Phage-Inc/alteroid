...
| 用途が増えるたびに `compose.yaml` へ環境変数を足す            | 人間が `.zshenv` に1行足すだけの作業が実装作業になる（デグレード）   | **名前と値**なら `alteroid credential set`、**スクリプト**なら `alteroid profile edit`。器を焼き直さない（⚠️ 鍵をプロファイルへ書くと、回した鍵が黙って巻き戻る） |
...
## Issue の「確かめていないこと」は、注意書きではなく仕事の指定である

**この repo の Issue はほぼ必ず「⚠️ 確かめていないこと」の節を持つ。そこは起票者が誠実に引いた線であって、そのまま残すべき留保ではない。** 起票者が「測れなかった」と書いた場所は、**測ればいちばん情報量の多い歯になる場所**である —— 起票者が排除できなかったものが、そこに名前付きで置いてある。

**⚠️ 「Issue の記述を鵜呑みにしない」とは別の話である。** あちらは**間違っているかもしれない主張**の扱いで、こちらは**正しいと分かっている欠落**の扱いである。前者は疑って確かめ直すが、後者は疑う余地が無い（起票者が自分で申告している）——**やるかやらないかだけが残る。**

**実測は2件で、どちらも同じ日（2026-09-17）の同じ引き継ぎの中に在る。**

| Issue が書いていた逐語                                                                               | 確かめた結果                                                                                                  |
| ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| #1050「⛔ **その形は再現していない**」（例外を出さずに片方の書き込みが踏み消される真の lost update） | `FsPersonaStore` の `append` で再現した。**その PR でいちばん強い歯になった**（変異で赤くなることまで取れた） |
| #1113「**fs でプロセスを跨いだ競合を再現していない。**」                                             | 本物の2子プロセスを `spawn` して区間の重なりを測る歯にした                                                    |

**⟹ どちらも、確かめる前は「直したことの証拠」が無かった。** 確かめた後は、**実装を元へ戻すと赤くなる**という形で証拠が付いた。

- **⛔ ただし「全部やれ」ではない。** 確かめるのに本番を壊す・人間の判断が要る・費用が釣り合わないものが在る（例: #1113 の「fs ストアで本番相当の運用をしている器が在るか」は、この層では確かめられない）。**やらないと決めたら、Issue を閉じるときに「どれを確かめて、どれを確かめなかったか」を分けて書くこと** —— 閉じた Issue の留保は、次に読む人からは「解決済み」と区別が付かない
- **確かめた結果が「起票者の見立てと違った」なら、それも成果である。** #1147 は「クローンが resume を諦めた原因」を説明する穴に見えたが、確かめると**本番（pg）ではその経路を踏まない**ことが分かり、代わりに器を問わない別の穴（#1157）が出た
...
- **検証コマンドを漏れなく列挙して渡す**（`build` / `check:web-bundle-node-traces` / `check:web-bundle-size` / `check:web-css-comment-classnames` / `apps/daemon/openapi.json` に差分が無いこと / `check:sdk-quotes` / `typecheck` / `lint` / `format:check` / `test`）。**渡さなかった検証は実行されない** — `typecheck` を渡し忘れて型エラー2件が CI まで残った
...
- **マネージャー・作業者は、クローンと同じ資格へ到達できる。これは未修正の穴ではなく、受容すると決めた状態である**（2026-09-16 の人間の決定。#888 を `not planned` で close。**`completed` ではない** — 門は立っていないからである）。**⛔ 塞ぎに行かないこと。** 知らずに手を付けると、既に機序で否定された案をもう一度やり直すことになる
  - **なぜ塞げないのか。** SDK の子プロセス（マネージャー・作業者・MCP・`Bash` の子）は**ただ1つの uid** で起こされる（`packages/core/src/runner.ts` の `spawnAsUser`。渡される `#childUser` は1つしか無い）ので、**ファイルの mode では「資格を読む主体」と「作業者」を分けられない。** 置き場を変えても（A-1）、ファイルを捨てて env だけにしても（A-2）塞がらず、**器の分類器と同じ門を作業者の層にも立てる案（A-3）は、同じ入力を1回目に拒んで2回目に通した実測が在るので「弾いていないこと」を測る歯が書けない。**⟹ 🔑 決定打は `gh` 自身が `gh auth token` を持つことで、**作業者が `gh` を打ってよい限り、鍵は届く**（#888 の逐語: 「**「`gh` を使わせる」と「`gh` の鍵を読ませない」は、同じ uid・同じプロセス族の中では両立しない**」）
  - **⟹ 境界はこの層の外（別 uid で `gh` を動かす＝新しい特権機構）にしか置けない。** それは M5（#485）/ M7（#486）でプロセス境界を引き直す機会が来たときに再検討する話で、**いまは着手しないと決めてある**
  - **⚠️ 「作業者から `gh` を取り上げる」で塞ぎに行かないこと。** `docs/north_star.md` の禁止2（追加制限禁止 —— 能力を削るのではなく、方針と実行環境の境界で表す）に正面から当たる
  - **⟹ だから上の3行は作法ではない。いま実際に効いている唯一の境界である。** 露出したときに外へ出るのは**クローンと同じ鍵**（オーナーが触れる repo すべてに届く）であり、**露出の大きさは人間と同じである**。⚠️ 一度実際に起きている（2026-09-12、作業者が自分のデバッグ用スクリプトで本物の値をトランスクリプトへ1回出した。**本人の自己申告で分かった** —— この申告できる状態が保たれていること自体が資産なので、壊さないこと）
  - 落とした案それぞれの機序と、軸2（読み出しの事後検出。**未実装のまま閉じた**）の扱いは #888 に在る（関連: #865 / PR #885 / PR #886）
...
- **閉じるキーワード（`close` / `fix` / `resolve` の活用形 + `#番号`）を、意図して閉じるとき以外は PR 本文にもコミットメッセージにも書かない。参照するだけなら番号だけを置く**（#1109）。閉じたいときは**キーワードと参照だけで構成された行**にする（または `gh issue close <N>` で手で閉じる）
  - **なぜ — 書いた側の意図は2通りの形で無視され、どちらも「閉じる」側へ倒れる**（実測 2026-09-16）。**(1) 日本語の範囲限定が効かない**: PR #1095 の本文末に置いた「閉じるキーワード + `#993` + 『の段1。』」は、GitHub には番号までしか読まれず、段2・段3 が残っていた #993 が**段1 のマージで閉じた**（マージ `12:11:28Z` → `closed` `12:11:30Z`、`commit_id` は `null`）。**(2) インラインコードスパンも除外されない**: PR #1107 の本文は、この誤爆を説明するためにバッククォートで囲んだ引用を2箇所持つだけで、裸のキーワードは1つも無く、本文自身が「この PR では閉じるキーワードを書いていない」と明言していた。**それでも閉じた**（マージ `16:46:17Z` → `closed` `16:46:19Z`、`commit_id` はその PR のマージコミット `f942230`）⟹ **この誤爆を報告すること自体が、同じ誤爆を起こす**
  - **そして閉じたことは PR の側に1文字も出ない。** CI は緑、マージは成功、PR のページに警告は出ない。気づく契機は「後で Issue を開いた人が閉じているのを見る」しかなく、そのとき見えるのは結果だけで、誰が・なぜ閉じたのかは timeline を引かないと分からない
  - **倒れる向きが両方とも「閉じる」側である** ⟹ 閉じたい意図は文面どおり通るのに、**閉じたくない意図はどう書いても文面からは守れない。** だから規約の側を「書かない」へ倒す
  - **公式 doc に在ること**（`linking-a-pull-request-to-an-issue`、2026-09-17 取得）: キーワードは9語（`close` / `closes` / `closed` / `fix` / `fixes` / `fixed` / `resolve` / `resolves` / `resolved`）、**コロンと大文字も有効**（逐語: `The keywords can be followed by colons or in uppercase.`）、**コミットメッセージでも効く**（逐語: `You can also use closing keywords in a commit message.`）、別 repo は `OWNER/REPOSITORY#番号`。**コードブロック・引用・HTML コメントを除外するという記述は無い**（上の (2) の実測と整合する）
  - **CI の門は `pr-closing-keywords`**（`.github/workflows/pr-closing-keywords.yml`。判定は `scripts/check-pr-closing-keywords-core.mjs` が正本で、通す形・落とす形の分類と理由はあちらの doc に在る）。**required contexts には入れていない** —— この門は書いた側の意図を読めないので、**どうしても本文にキーワードが要る回に「赤いまま進める」ことが逃げ道そのものである。** required にするなら、逃げ道（ラベル等）を同時に設計すること
  - **実測（2026-09-17 観測）**: この判定を直近のマージ済み PR 150本の本文へ当てると、キーワード + 参照を持つのは **32本**、**そのうち赤くなるのは5本**（#1095 / #1107 / #1070 / #912 / #868）で、**残る27本は通る**（キーワードと参照だけの行、または強調で囲んだ `**…**` の形）。⟹ **意図して閉じている形は素通りし、意図と GitHub の読みが食い違う形だけが落ちる**
  - **⚠️ 訂正されていない3例目が在る。** PR #912 の本文は「閉じるキーワード + `#910` + 『の候補（draft。依頼者の合図待ち。マージも issue の close もしていない）』」と書いているが、**#910 はその PR のマージの1秒後に閉じている**（マージ `2026-09-13T01:03:46Z` → `closed` `01:03:47Z`、`commit_id` は `null`。reopen は無く**いまも CLOSED**）。**「誤って閉じた」とは言えない** —— マージ時点では閉じてよかった可能性が在り、そこは測っていない。**言えるのは「本文が『close していない』と書いている隣で、閉じたのは人の判断ではなくキーワードだった」ところまでである**
  - **⚠️ 規約のタイトルの型そのものが、この罠の形をしている。** 公式 doc はキーワードの後ろに**コロン**を許し、キーワードには **`fix`** が在る ⟹ **`fix: #123 の…` という形のタイトルは、doc の形からは「閉じる指示」として読める**（**実測はしていない** —— 測るには本番の Issue を実際に閉じるしかない）。**そして squash マージが件名に焼くのは PR のタイトルである** ⟹ 件名経由でも閉じうる。実測（2026-09-17、直近マージ済み150本）: タイトルにキーワード + 参照の**隣接**を持つものは **0本**。ただし PR #1017 のタイトル `docs: #787範囲外の出典腐り2件を直す（latest_per_family / 296行）` は**1文字違いで踏める形**である（`docs` はキーワードではないので無害だった）。**「型 + 説明 +（#番号）」の形（`fix: … を畳む（#1041）`）は隣接していないので当たらない** —— PR #1112 のマージの2秒後に #1041 が閉じたのは、タイトルではなく**本文に在ったキーワードと参照だけの行**が原因である（確認済み。これは意図どおりの閉じ方で、門も通す形）
...
- **出自の刻印（`<!-- alteroid-origin: … -->`）は付けない**（2026-09-15 の決定。#893）。PR / Issue の本文に層の出自を刻む機構が在ったが、PR #947 が本文に一言も書かずに消し（記録は #971）、#893 がそれを追認して「やめる」を採った。**根拠は費用の見積もりではなく実測である** — クローンは落ちた委譲の成果を10回近く探して、**一度も刻印を使わなかった**（使ったのは `gh pr list` / `git ls-remote` / `gh api .../check-runs` / 日誌）。⟹ **「不要と決めた」ではなく「使われていないと測れた」。層の判別は日誌・委譲の台帳の側で行う** — 直上の項と同じ向きである
  - **⚠️ 同じ提案をやり直さないこと。** 「機構が消えた」と「付けないと決めた」は別の事実で、**`grep` が0件なのは未規約だからではなく決定の結果である**（#1046）。⚠️ **決定の後も慣行は止まらなかった** — 機構が消えた後の Issue のコメントに刻印が付き続けている（#1046 の実測、2026-09-16 観測で4件）。**⟹ 規約に書くことは必要条件であって、十分条件ではない**
  - **⚠️ やめたことで追えなくなるものが在る。** 「外から来た枝か」の検出は刻印が無くても代替が効く（**#1027 の実測**: 「`main` と共通祖先を持たない」という判定が28枝すべてで誤検知0・取りこぼし0）。**代替が効かないのはその先で、「どの委譲が運んだか」「向こう側のどのコミットに対応するか」「事故か、意図された中継か」は刻印でしか取れていない**（同じく **#1027 の実測**。刻印が結論を確定させた実例が在り、#893 に事実として記録されている）。**それでも決定は「やめる」である。見直すかどうかを決めるのは人間である**
...
- **デプロイの引き金はマージではない。** Railway が見るのは `release/prod` で、そこへ `main` が写されるのは**1日1回、夜だけ**である（`.github/workflows/release-prod.yml` の `cron: '17 19 * * *'` ＝ UTC 19:17 ＝ **JST 翌 4:17**）。**⚠️ 2026-09-05 に2回から1回へ減った**（`088a2b2`。それまでは UTC 13:17 / 19:17 の2回で、ここには「1日2回、JST 22:17 と翌 4:17」と書いてあった）。**設定どおりの時刻には発火しない** —— 実測（2026-09-15T19:40Z 観測。**単一 cron へ移った 2026-09-05 以降**の `event=schedule` 連続10回）で遅れは **2時間02分〜3時間16分**（中央値 2時間27分）⟹ 実際に走るのは **JST 06:19〜07:33** の幅である。**⚠️ cron の設定が変わったら、それ以前の実測を混ぜないこと。** ここには「13分〜2時間37分 ⟹ **JST 04:29〜06:54**」と書いてあったが、その標本には**2回設定だった頃の run が混じっていて**、下限が1時間以上低く出ていた。**⚠️ そして数字は腐る。腐っても赤くならないので、使う前に自分で測り直すこと** —— `gh run list --repo takecchi/alteroid --workflow=release-prod.yml --limit 100 --json createdAt,event,conclusion` を打ち、`event=schedule` の行だけを見る（`workflow_dispatch` を混ぜると窓が広く見える）。**だからマージしても、その瞬間に走行中のマネージャーと作業者が畳まれることはない。** 器が入れ替わればそれらは畳まれるが、その時刻は**自分の操作と無関係に**来る（「マージする前に確かめる」では守れなくなった代わりに、「自分のマージで自分が死ぬ」が無くなった）。落ちる Service は `railway/daemon.json` / `railway/runner.json` の `watchPatterns` で決まり、`docs/` や `.gitignore` だけの PR ではどちらも落ちない。範囲の内訳と、被害を減らすために置いてある仕組みは [railway/README.md](./railway/README.md)「デプロイは走行中の仕事を畳む操作である」
...
- **同一の git 作業ツリーを複数のプロセスが同時に書き換えることがある。ID だけでは器は分離できない — ID が衝突しうる。** マネージャー（依頼者）が3体の作業者へ「`/workspace/<自分のID>/` へ clone せよ」と指示したところ、**3体の ID がすべて `worker-1aa8c87d` で同一**だったため、3体が `/workspace/worker-1aa8c87d/repo` という同一の `.git` を共有した。**これは上の worktree の話（プロセス空間の共有）とは別の軸で、今回はファイルすら分かれていなかった。** 作業者の1体が報告した生の経緯 — clone 直後の `git status` は clean だった。`AGENTS.md` を編集した直後に取った `git status` には、自分が触っていない `apps/daemon/src/schedule.ts` に26行の未コミット差分が既にあった（別の作業者の作業）。`git checkout -b docs/cd-fg-bg-cwd-lifetime` の数十秒後には `.env.example` / `compose.yaml` / `railway/README.md` にも未コミット差分が出現していた。**`git commit` を実行したところ、出力が `[fix/initiative-tick-default-55 4421d5b] …` になった** — 自分が作ったブランチではなく、別の作業者のブランチへコミットされていた（`checkout -b` の後に別プロセスが HEAD を奪っていた）。**⭐ 最後に、他プロセスへブランチを返すつもりで `git checkout fix/initiative-tick-default-55` を試みたところ、git 自身が「`AGENTS.md` の未コミットのローカル変更が上書きされる」として checkout を拒否し、この拒否のおかげでデータが失われなかった。これは仕組みで塞げているのではなく、たまたま git のこの挙動に助けられた形である。** 「`/workspace/<自分のID>/` へ clone し、既存のツリーには触らない」は ID が一意である前提の上でしか効かない。**ID の一意性は自分では確かめられないので、渡された ID をそのまま使う前に衝突の可能性を考えること**
  ...
  - **退避（停止に備えて成果を出す）が要るときに、持ち主でない側が撃ってよい git / 撃ってはいけない git を分ける。**
    ...
    - ⚠ ただし `git worktree add` は共有の `.git/` へ書く（`grep -Fn -- '設定に依存する検査は、worktree を分けても切り分けられない' AGENTS.md` が既に持つ）。中身・index・HEAD は分かれるので⛔には当たらないが、完全に無干渉ではない。いちばん干渉が少ないのは別 clone である
...
- `pnpm install` → `pnpm build` → `pnpm check:web-bundle-node-traces` / `pnpm check:web-bundle-size` / `pnpm check:web-css-comment-classnames` / `apps/daemon/openapi.json` に差分が無いこと / `pnpm check:sdk-quotes` / `pnpm typecheck` / `pnpm lint` / `pnpm format:check` / `pnpm test`
...
- **Claude 本体（`@anthropic-ai/claude-agent-sdk`）の版を手で上げに行かないこと。** `.github/workflows/update-claude-sdk.yml` が毎日見て、上がっていれば1本の PR（`automation/claude-agent-sdk`）に積み直す。**その PR は AI の層が自分の判断でマージしてよい**（2026-09-16 の人間の決定。#1053 — ここには「マージするのは人間である」と書いてあり、PR #1044 を AI の層がマージしたことをきっかけに人間へ諮って置き換えた）
